...
if($nilaiTahun != 0 ){
  header("Content-Disposition: attachment; filename=Data Laporan Beban BPLL ".$nilaiTahun.".xls");
}else{
  header("Content-Disposition: attachment; filename=Data Laporan Beban BPLL .xls");
}
header("Pragma : no-cache");
header("Expires :0"); $i=0;
?>

 <div align="center"> <p>Data Laporan Beban BPLL <?php if($nilaiTahun >0 ) ?> Tahun <?php { echo $nilaiTahun;} ?> <?php if($nilaiTriwulan == 0){?> Semua Triwulan <?php  }else{ ?> Triwulan <?php echo $nilaiTriwulan;} ?> </p></div>

                       <table  border="1"    class="table table-striped table-bordered " class="centered">
                        <thead>
                          <tr bgcolor="#ddebf7">
